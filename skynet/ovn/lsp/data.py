...
class LogicalSwitchPort():
    ...
    """
    def __init__(self, lsp_data: List[Dict[str, Any]],
                 both_data: List[Dict[str, Any]]):
        """
        ...
        self._lrp = LRPData(
            list(
                filter(
                    lambda d: d['Metadata']['Type'] == 'logical_router_port',
                    both_data)))
        self._pod = PodData(
            list(filter(lambda d: d['Metadata']['Type'] == 'pod', both_data)))
        self._iface = OvSIfaceData(
            list(filter(lambda d: d['Metadata']['Type'] == 'veth', both_data)))
...
class LSPProvider(SkyDiveDataProvider):
    ...
    def get(self, lsp: str) -> LogicalSwitchPort:
        ...
        """
        query = "V().Has('Type', 'logical_switch_port', 'UUID', '{lsp}')".format(
            lsp=lsp)
